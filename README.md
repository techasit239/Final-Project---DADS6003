...
_แผนภูมิที่ 2 Top 10 feature ที่มีอิทธิพลต่อ Model XGBoost (รวม Advance Feature)_   
   ...
   **สรุปผลลัพธ์**
   - สรุปผลลัพธ์ Experiment A จากการทดลอง Experiment A ที่ได้นำเสนอไอเดียใหม่โดยการผนวก 8 Advanced Features เข้ากับ Stylometric Features เดิม พบว่าโมเดล XGBoost ยังคงครองความเป็นเลิศแต่มีประสิทธิภาพที่สูงขึ้นอย่างชัดเจน โดยทำค่า F1-score ได้ถึง 88.00% (สูงกว่างาน Reproduce ที่ทำได้ 83.33%) พร้อมค่า Precision ที่สูงถึง 91.67% ซึ่งจุดเปลี่ยนสำคัญอยู่ที่การเพิ่มฟีเจอร์ด้านพฤติกรรม (Behavioral Features) เข้ามา โดยเฉพาะ Action_Time_Coercion_Density ที่กลายเป็นฟีเจอร์ที่มีอิทธิพลสูงสุด (Feature Importance 15.9%) แซงหน้าฟีเจอร์เดิมๆ บ่งชี้ว่าโมเดลสามารถเรียนรู้ "เจตนาการบีบคั้น" ของคนร้ายที่ต้องการให้เหยื่อรีบกระทำบางอย่างได้ และเมื่อทำงานร่วมกับสัญญาณการตะโกนกดดัน (upper_ratio) และการสร้างฟอร์มทางการปลอม (char_freq_[) ทำให้โมเดลยกระดับจากการมองแค่ "สไตล์การเขียน" มาเป็นการจับผิดที่ "กลยุทธ์การหลอกลวง" (Deception Tactics) ส่งผลให้สามารถคัดกรองอีเมล Phishing ได้อย่างแม่นยำและครอบคลุมยิ่งขึ้นกว่างานวิจัยเดิม
...
_แผนภูมิที่ 3 Top 10 feature ที่มีอิทธิพลต่อ Model SVM (รวมตัวแปรจากการทำ TF-IDF)_  
   ...
   **สรุปผลลัพธ์**
   - จากการทดลอง Experiment B ที่เปลี่ยนแนวทางมาใช้เทคนิค NLP ด้วยการสกัดคำศัพท์สำคัญ (TF-IDF) เพียงอย่างเดียว พบว่าเกิดจุดเปลี่ยนสำคัญที่ทำให้โมเดลกลุ่มเส้นตรง (Linear) อย่าง SVM พลิกกลับมาเป็นผู้ชนะขาดลอยด้วยค่า F1-score สูงถึง 96.30% และที่สำคัญคือทำค่า Recall ได้เต็ม 100% (ไม่ปล่อยให้ Phishing หลุดรอดไปได้แม้แต่ฉบับเดียว) ซึ่งเอาชนะโมเดลตระกูล Boosting ที่เคยเป็นแชมป์เก่าได้อย่างราบคาบ สาเหตุเพราะข้อมูลแบบข้อความ (Text) ที่ถูกแปลงเป็น TF-IDF นั้นมีลักษณะเป็นมิติที่กว้างและกระจายตัว ซึ่งเข้าทางโมเดลอย่าง SVM และ Logistic Regression ที่เก่งฉกาจในการขีดเส้นแบ่ง (Hyperplane) เพื่อแยกแยะคำศัพท์อันตรายอย่าง 'organization', 'xyz' หรือ 'solutions' ได้อย่างเด็ดขาดและแม่นยำ ในขณะที่โมเดลซับซ้อนอย่าง Decision Tree หรือ Boosting กลับทำผลงานได้ดรอปลงเพราะข้อมูลที่มีขนาดเล็ก (Small Data) ทำให้เกิดสัญญาณรบกวนได้ง่าย จึงสรุปได้ว่าสำหรับโจทย์นี้ การจับผิดที่ "เนื้อหา (Content)" เป็นวิธีการที่ทรงพลังและแม่นยำที่สุด
...
_แผนภูมิที่ 4 Top 10 feature ที่มีอิทธิพลต่อ Model Random Forest (รวมตัวแปรจากการทำ TF-IDF และ 8 Advance Feature)_  
   ...
   **สรุปผลลัพธ์**
   - จากการทดลอง Experiment C ที่เป็นการ "ยำรวมมิตร" เอาฟีเจอร์ทุกอย่างมารวมกัน (Stylometric + Advanced + TF-IDF) พบว่าแชมป์เปลี่ยนมือมาเป็น Random Forest ซึ่งทำคะแนน F1-score ได้สูงสุดถึง 92.31% (ชนะ SVM ที่เคยเป็นแชมป์รอบที่แล้ว) เหตุผลหลักคือเมื่อเราเอาข้อมูลหลายประเภทมาผสมกัน ทั้งแบบนับคำ (TF-IDF) แบบสถิติ (Stylometric) และแบบพฤติกรรม (Advanced) ข้อมูลจะมีความ "หลากหลายและซับซ้อน" สูงมาก ซึ่งเข้าทางโมเดลตระกูลต้นไม้ (Tree-based) อย่าง Random Forest ที่เก่งเรื่องการจัดการข้อมูลผสมๆ กันและมีความเสถียรสูง (Robust) ไม่ค่อยหวั่นไหวกับ Noise หรือฟีเจอร์ที่เยอะเกินไป ต่างจาก SVM หรือ Logistic Regression ที่พอเจอฟีเจอร์ปนกันเยอะๆ กลับทำผลงานตกลงไปเหลือแค่ ~76-80% เพราะเริ่มสับสนในการขีดเส้นแบ่ง โดยเมื่อดูไส้ในของฟีเจอร์ที่ Random Forest เลือกใช้ ก็พบว่ามีการผสมผสานกันอย่างลงตัวระหว่าง คำศัพท์ ('organization'), พฤติกรรมการบีบคั้น ('Action_Time_Coercion'), และสัญลักษณ์แปลกๆ (']') แสดงให้เห็นว่าการรวมพลังฟีเจอร์ช่วยให้โมเดลมองเห็นภาพรวมได้ดี แต่ต้องเลือกโมเดลที่ "จัดการความยุ่งเหยิง" ได้เก่งอย่าง Random Forest ถึงจะรีดประสิทธิภาพออกมาได้ดีที่สุด
...
<img width="1050" height="350" alt="{C9F95754-EC37-4004-88BE-16D0B46F997C}" src="https://github.com/user-attachments/assets/8058e539-3d1a-44bc-8e5f-e71992b0b681" />
   ...
   **ข้อเสนอแนะ**
   - แนวทางพัฒนาต่อในอนาคต (Future Work) แทนที่จะมุ่งเน้นการหา Feature เชิงโครงสร้างที่ซับซ้อน ควรหันมาลงทุนกับเทคนิค NLP ขั้นสูง เช่น การใช้ Word Embeddings หรือ Transformer-based models (เช่น BERT) ที่สามารถเข้าใจ "บริบท" และ "ความหมายแฝง" ของประโยคได้ดียิ่งขึ้น เพื่อดักทาง Phishing ยุคใหม่ที่อาจเปลี่ยนคำศัพท์หลบเลี่ยง Keyword เดิมๆ แต่ยังคงเจตนาหลอกลวงแบบเดิม
